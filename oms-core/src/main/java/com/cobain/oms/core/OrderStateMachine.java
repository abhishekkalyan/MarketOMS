...
public final class OrderStateMachine {
     ...
     * ROUTING means: order accepted, children dispatched, awaiting fills.
     *
     * Only valid from NEW state. Idempotent if already ROUTING.
     * Returns false if the current state is neither NEW nor ROUTING.
     *
     * ZERO-ALLOCATION: single byte read + write.
     */
    public static boolean transitionToRouting(final com.cobain.oms.model.OrderFlyweight parent) {
        final byte currentState = parent.orderState();
        if (currentState == ParentOrderState.ROUTING) {
            return true; // already routing — idempotent
        }
        if (currentState != com.cobain.oms.model.OrderState.NEW) {
            return false; // invalid: only NEW → ROUTING is permitted
        }
        parent.orderState(ParentOrderState.ROUTING);
        parent.transactTime(System.nanoTime());
        return true;
    }
